...
class MetaLearner(ABC):
    def __init__(
        ...
        func_calc_metrics: Callable[
            [list[NDArray], list[NDArray]], tuple[dict, str, str]
        ],
    ):
        self.config = config
        ...
        self.output_path = os.path.join(
            self.config["save"]["output_path"], self.config["save"]["exp_name"]
        )
        self.ckpt_path = os.path.join(
            self.config["save"]["ckpt_path"], self.config["save"]["exp_name"]
        )
    ...
    def learn(self):
        ...
        for epoch in range(curr_epoch, self.config["learn"]["num_epochs"] + 1):
            ...
            if (
                epoch % self.config["learn"]["tune_freq"] == 0
    ...
    def tune_train_test(
        ...
        labels, preds, names = self.tune_train_test_process(
            tune_train_loader, tune_test_loader, epoch, sparsity_mode
        )
    ...
    def run_sparse_tuning(self, epoch: int):
        ...
        for sparsity_mode in sparsity_modes:
            for tl in self.tune_loader[sparsity_mode]:
                ...
                if sparsity_mode == "dense":
                    self.print_and_log(
                        'Evaluating "%s" (%d-shot) ...' % (sparsity_mode, tl["n_shots"])
                    )
                else:
                    self.print_and_log(
                        'Evaluating "%s" (%d-shot, %d-%s) ...'
    ...
    def read_checkpoint(self):
        with open(
            os.path.join(self.ckpt_path, FILENAMES["checkpoint"]), "r"
        ) as ckpt_file:
    ...
    def save_all_tune_mask(self, tune_loader: DataLoader, sparsity_mode: str):
        ...
        for i, data in enumerate(tune_loader):
            ...
            for j in range(len(img_name)):
                ...
                stored_sparse = (stored_sparse * (255 / stored_sparse.max())).astype(
                    np.uint8
                )
                io.imsave(
                    os.path.join(
    ...
    def save_net_and_optimizer(self, epoch: int = 0):
        ...
        torch.save(
            self.meta_optimizer.state_dict(),
