...
class SimpleLearner(BaseLearner):
    ...
    def learn_process(self, epoch: int):
        ...
        for i, train_data in enumerate(self.dataset_loader['train']):
            ...
            self.print_and_log('Train Ep: %d/%d, it: %d/%d, loss: %.4f' %
                               (epoch, num_epochs, i + 1, train_loader_len, detached_loss))
        train_avg_loss = total_train_loss / train_loader_len
        self.print_and_log('Train Ep: %d/%d, avg loss: %.4f' %
                           (epoch, num_epochs, train_avg_loss))
        ...
        self.print_and_log('Val Ep: %d/%d, avg loss: %.4f' %
                           (epoch, num_epochs, val_avg_loss))
        ...
        self.write_to_csv(
            ...
            ['epoch', 'train_duration', 'val_duration', 'post_gpu_percent', 'train_avg_loss', 'val_avg_loss']
            + sorted(score.keys()),
            row
        )
    ...
    def test_process(self, epoch: int):
        ...
        with torch.no_grad():
            ...
            for i, test_data in enumerate(self.dataset_loader['test']):
                ...
                test_preds.append(p_ts.detach().max(1)[1].squeeze(1).squeeze(0).cpu().numpy())
                test_names.append(img_name[0])

                self.print_and_log('Test Ep: %d, it: %d/%d' % (epoch, i + 1, test_loader_len))
        ...
        self.write_to_csv(
            ...
            ['dataset', 'epoch', 'duration', 'post_gpu_percent']
            + sorted(score.keys()),
            row
        )
    ...
    def save_prediction(self, prediction: NDArray, filename: str):
        ...
        io.imsave(
            os.path.join(self.output_path, FILENAMES['prediction_folder'], f'{filename}.png'),
            stored_prediction)
