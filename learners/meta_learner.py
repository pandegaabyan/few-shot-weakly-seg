...
class MetaLearner(BaseLearner, ABC):
    def __init__(
        ...
        self.meta_loaders = get_meta_loaders(
            self.meta_params, config["data"], pin_memory=config["learn"]["use_gpu"]
        )
    ...
    def meta_train_test(self, epoch: int):
        ...
        for i, ml in enumerate(self.meta_loaders):
            ...
            for j, train_data in enumerate(ml["train"]):
                ...
                self.print_and_log(
                    "Ep: %d/%d, data: %d/%d, it: %d/%d, loss: %.4f"
                    % (
                        epoch,
                        num_epochs,
                        i + 1,
        ...
        # Printing epoch loss.
        self.print_and_log(
            "Ep: %d/%d, total loss: %.4f" % (epoch, num_epochs, total_loss), end="\n"
        )
