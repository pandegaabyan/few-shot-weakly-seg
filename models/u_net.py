    ...
    for model in models:
        for module in model.modules():
            if (isinstance(module, nn.Conv2d) or isinstance(module, nn.Linear) or
                    isinstance(module, modules.MetaConv2d) or isinstance(module, modules.MetaLinear)):
...
class MetaConvTranspose2d(nn.ConvTranspose2d, modules.MetaModule):
    ...
    def forward(self, input, output_size=None, params=None):
        ...
        output_padding = self._output_padding(
            input, output_size, self.stride, self.padding, self.kernel_size, 2, self.dilation) # type: ignore

        return F.conv_transpose2d(
            input, weights, bias, self.stride, self.padding, # type: ignore
