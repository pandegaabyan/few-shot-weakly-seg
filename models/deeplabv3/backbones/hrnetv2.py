    ...
    def forward(self, x):
        ...
        for branch_output_index in range(
            ...
            for input_index in range(
                ...
                if input_index == 0:
                    x_fused.append(
                        self.fuse_layers[branch_output_index][input_index](  # type: ignore
                            x[input_index]
                        )
                    )
                else:
...
def hrnetv2_48(number_blocks=[1, 4, 3], **kwargs):
    w_channels = 48
    return HRNet(c=w_channels, num_blocks=number_blocks, **kwargs)


def hrnetv2_32(number_blocks=[1, 4, 3], **kwargs):
    w_channels = 32
    return HRNet(c=w_channels, num_blocks=number_blocks, **kwargs)
