...
class DiscCupLoss(CustomLoss):
    ...
    def forward(self, inputs: Tensor, targets: Tensor):
        ...
        if self.loss_type == "iou":
            od_input, oc_input, od_target, oc_target = self.process_input_and_target(
                inputs, targets
            )
        ...
        if self.loss_type == "iou_bce":
            ...
            od_input, oc_input, od_target, oc_target = self.split_od_oc(inputs, targets)
            od_loss_bce = self.bce_loss(
                od_input, od_target, ignore_index=self.ignored_index
            )
            oc_loss_bce = self.bce_loss(
                oc_input, oc_target, ignore_index=self.ignored_index
            )
    ...
    def process_input_and_target(
        self, inputs: Tensor, targets: Tensor
    ) -> tuple[Tensor, Tensor, Tensor, Tensor]:
        soft_inputs = functional.softmax(inputs, 1)

        od_input, oc_input, od_target, oc_target = self.split_od_oc(
            soft_inputs, targets
        )
